...
@push('head-scripts')
    <script src="{{asset('js/pages/handle-person.js')}}" defer></script>
...
@section('content')
    ...
    <main class="safe-area py-4">
        <form method="POST" action="{{$route}}" class="handle-person_form d-flex flex-column gap-1">
            @csrf
            @method($method)
            ...
            <button type="submit" class="w-100 normal-btn btn-with-icon mt-3">
                Salvar
